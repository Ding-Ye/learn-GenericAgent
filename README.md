...
| s02 | s02-tools | 工具注册表与 dispatch | `agent_loop.py:BaseHandler` | ⏳ |
| s03 | s03-outcome | StepOutcome 控制流 | `agent_loop.py:StepOutcome` | ⏳ |
| s04 | s04-claude | 真实 Anthropic Claude provider | `llmcore.py:NativeClaudeSession` | ⏳ |
| s05 | s05-coderun | 流式代码执行工具 | `ga.py:code_run` | ⏳ |
| s06 | s06-fileops | 文件读写补丁工具 | `ga.py:file_read/write/patch` | ⏳ |
| s07 | s07-memory | 分层记忆与 working checkpoint | `memory/`, `ga.py:do_update_working_checkpoint` | ⏳ |
| s08 | s08-skills | 技能树与技能搜索 | `memory/skill_search/SKILL.md` | ⏳ |
| s09 | s09-mixin | 多 provider 故障切换 | `llmcore.py:MixinSession` | ⏳ |
